...
/**
 * renderPlayer - Render the player postion and line of sight on the mini-map.
 *
 * @state: The state of the game.
 * @player: The player.
 * @miniMapSize: The size of the mini-map.
 */
void renderPlayer(State *state, Player *player, int miniMapSize)
{
	...
	SDL_SetRenderDrawColor(state->renderer, 255, 255, 0, 100); /*yellow*/
...
void renderMiniMap(State *state, Player *player, int miniMapSize)
{
	...
	/*Render the player position & line of sight on the mini-map*/
	renderPlayer(state, player, miniMapSize);
